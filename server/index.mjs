 ...
 *
 * It exposes an OpenAI-compatible streaming endpoint so the React client
 * (src/lib/llm.ts) can talk to it with a plain fetch + SSE parser:
 *
 *   POST /v1/chat/completions   { model, messages, stream: true }  -> SSE
 *   GET  /health
 *
 * Run:  node server/index.mjs   (see package.json "server" / "dev:all" scripts)
 */
...
for (const file of [".env.local", ".env"]) {
  if (fs.existsSync(file)) {
    ...
    console.log(`[pointpilot] loaded env from ${file}`);
    break;
  }
}
...
async function configureProxy() {
  ...
  console.log(`[pointpilot] proxy tunneling enabled via ${proxy}`);
}
...
const PORT = Number(process.env.PORT ?? process.env.POINTPILOT_PORT ?? 8787);
...
if (!API_KEY) {
  console.warn(
    "[pointpilot] WARNING: CURSOR_API_KEY is not set. Add it to .env " +
      "(get a key at https://cursor.com/dashboard/integrations).",
  );
}

// Scratch workspace for the local agent (it runs against a cwd).
const SCRATCH = fs.mkdtempSync(path.join(os.tmpdir(), "pointpilot-"));
...
function toPrompt(messages) {
  ...
  if (convo.length === 0) {
    ...
    for (const m of convo) {
      const who = m.role === "user" ? "User" : "PointPilot";
      lines.push(`${who}: ${m.content}`);
    }
  }
  lines.push(
    "\nRespond as PointPilot to the user's latest message. Follow your system " +
    ...
    const message =
      err instanceof CursorAgentError
        ...
        : err instanceof Error
          ? err.message
          : "Unknown error";
    console.error("[pointpilot] chat error:", message);
    if (!sent) {
...
const server = http.createServer((req, res) => {
  ...
  if (req.method === "POST" && req.url?.startsWith("/v1/chat/completions")) {
    handleChat(req, res).catch((err) => {
      console.error("[pointpilot] fatal:", err);
      if (!res.headersSent) {
        res.writeHead(500, { "Content-Type": "application/json" });
...
server.listen(PORT, () => {
  console.log(`[pointpilot] backend listening on http://localhost:${PORT}`);
  console.log(`[pointpilot] model: ${MODEL} | key: ${API_KEY ? "configured" : "MISSING"}`);
});
